...
// open
// ----------------------------------------------------------
bool SimpleJSONWriter::open(const char* fileName) {
	f = fopen(fileName, "w");
	_started = true;
	return true;
}
...
// start category
// ----------------------------------------------------------
void SimpleJSONWriter::startCategory(const char* name) {
	if (!_started) {
		fprintf_s(f, "\n");
	}
	writeIdent();
...
// write int
// ----------------------------------------------------------
void SimpleJSONWriter::write(const char* name, int value) {
...
// write uint32
// ----------------------------------------------------------
void SimpleJSONWriter::write(const char* name, uint32 value) {
...
// write float
// ----------------------------------------------------------
void SimpleJSONWriter::write(const char* name, float value) {
...
// write string
// ----------------------------------------------------------
void SimpleJSONWriter::write(const char* name, const char* value) {
...
// write boolean
// ----------------------------------------------------------
void SimpleJSONWriter::write(const char* name, bool value) {
...
// write v2
// ----------------------------------------------------------
void SimpleJSONWriter::write(const char* name, const v2& value) {
...
// write v3
// ----------------------------------------------------------
void SimpleJSONWriter::write(const char* name, const v3& value) {
...
// write color
// ----------------------------------------------------------
void SimpleJSONWriter::write(const char* name, const ds::Color& value) {
...
// write rect
// ----------------------------------------------------------
void SimpleJSONWriter::write(const char* name, const ds::Rect& value) {
...
// end category
// ----------------------------------------------------------
void SimpleJSONWriter::endCategory() {
	_items = 0;
	fprintf_s(f, "\n");
	_ident -= 4;
	writeIdent();
...
// write line ident
// ----------------------------------------------------------
void SimpleJSONWriter::writeLineIdent() {
	fprintf(f, "\n");
	for (int i = 0; i < _ident; ++i) {
		fprintf(f, " ");
	}
...
// write color path
// ----------------------------------------------------------
void SimpleJSONWriter::write(const ds::ColorPath& path) {
	for (int i = 0; i < path.size(); ++i) {
		const ds::Color& value = path.value(i);
		float key = path.key(i);
		writeLineIdent();
...
// write color path
// ----------------------------------------------------------
void SimpleJSONWriter::write(const ds::Vector2fPath& path) {
	for (int i = 0; i < path.size(); ++i) {
		const v2& value = path.value(i);
		float key = path.key(i);
		writeLineIdent();
...
// write color path
// ----------------------------------------------------------
void SimpleJSONWriter::write(const ds::FloatArray& path) {
	for (int i = 0; i < path.size(); ++i) {
		float value = path.value(i);
		float key = path.key(i);
		writeLineIdent();
...
// write ident
// ----------------------------------------------------------
void SimpleJSONWriter::writeIdent() {
	for (int i = 0; i < _ident; ++i) {
		fprintf(f, " ");
	}
}