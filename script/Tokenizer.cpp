	...
	while ( *p != 0 ) {    
		...
		if ( *p >= '0' && *p <= '9' ) {
			...
			if ( t.type == Token::UNKNOWN ) {
				...
				VarType varType = DT_FLOAT;
				if ( cnt > 0 ) {
					Token prev = tokens[cnt-1];
					if ( prev.type == Token::DECLARATION ) {
						varType = env.getDeclarationType(prev.id);
					}
				}
				if ( cnt > 1 ) {
					Token prev = tokens[cnt-2];
					if ( prev.type == Token::EXTERN ) {
						externVar = true;
					}
				}
				if ( externVar ) {
					uint32 id = env.addSymbol(hash,varType);
					t = Token(Token::SYMBOL,id);
				}
				else {
					if ( varType == DT_INT ) {
						uint32 id = env.addVariable(hash,0);
						t = Token(Token::NAME,id);
					}
					else if ( varType == DT_VEC2 ) {
						Vector2f v;
						uint32 id = env.addVariable(hash,v);
						t = Token(Token::NAME,id);
					}
					else if ( varType == DT_FLOAT ) {
						uint32 id = env.addVariable(hash,0.0f);
						t = Token(Token::NAME,id);
					}
					else {
						t = Token(Token::NAME);
		...
		if ( t.type != Token::UNKNOWN && t.type != Token::SEMICOLON) {
			if ( cnt < maxTokens ) {
				tokens[cnt++] = t;
			}
		}
...
bool Tokenizer::run(const uint32 *byteCode,ScriptContext& context,Stack &stack,bool debug) {
	...
	while (true) {
		...
		if ( debug ) {
			if ( op == BC_FUNCTION ) {
				LOG << "op: " << translateOpCode(op) << " id: " << context.getFunction(id).name;
			}
			else {
				LOG << "op: " << translateOpCode(op) << " id: " << id;
			}
		}
		switch (op) {
			case BC_DECLARATION:
			...
			case BC_SYMBOL: 
				stack.push(context.getSymbol(id));
				break;
			case BC_NUMBER:
...
const char* Tokenizer::translateOpCode(uint32 opCode) {
	switch ( opCode ) {
		...
		case 2146435072 : return "BC_SYMBOL";break;
...
uint32 Tokenizer::compile(const char* p, ScriptContext& context, std::vector<ScriptBlock>& blocks,bool debug) {
	...
	if ( debug ) {
		...
		for ( int l = 0; l < lines; ++l ) {
			LOG << l << " - program counter " << programmCounter[l];
		}
		for ( int l = 0; l < lines - 1; ++l ) {
			LOG << "tokens detected: " << lines;
			for ( int i = programmCounter[l]; i < programmCounter[l+1]; ++i ) {
				if ( tokens[i].type != Token::FLOAT ) {
					LOG << i << " : " << context.translate(tokens[i]) << " (" <<tokens[i].id << ")";  
				}
				else {
					LOG << i << " : " << context.translate(tokens[i]) << " (" << tokens[i].value << ")";  
				}
			}
		}
	}
		...
		if ( debug ) {
			...
			for ( int i = 0; i < rpls; ++i ) {
				if ( rpl[i].type == Token::FLOAT ) {
					LOG << i << " : FLO " << context.translate(rpl[i]) << " " << context.getData(rpl[i].id);  
				}
				else if ( rpl[i].type == Token::FUNCTION ) {
					LOG << i << " : FUNC " << context.getFunction(rpl[i].id).name;  
				}
				else {
					LOG << i << " : " << context.translate(rpl[i]) << " " << rpl[i].id;  
				}
			}
		}
		ScriptBlock block;
...
void Tokenizer::saveByteCode(const char* name,std::vector<ScriptBlock>& blocks,ScriptContext& env ) {
	BinaryWriter writer;
	int signature[] = {0,8,15};
	writer.open(name,signature,3);
	for ( uint32 i = 0; i < env.numSymbols(); ++i ) {
		writer.startChunk(4,1);
		int varType = env.getSymbolType(i);
		writer.write(varType);
		writer.write(env.getSymbolHash(i));
		writer.closeChunk();
	}
...
void Tokenizer::loadByteCode(const char* fileName,std::vector<ScriptBlock>& blocks,ScriptContext& env) {
	...
	while ( loader.openChunk() == 0 ) {
		if ( loader.getChunkID() == 4 ) {			
			...
			IdString hash;
			loader.read(&hash);
			env.addSymbol(hash,(VarType)type);
			printf("adding symbol type %d = %d\n",type,hash);
		}
			...
			if ( type == 0 ) {
				env.addVariable(hash,0);
			}
			if ( type == 1 ) {
				env.addVariable(hash,0.0f);
			}
			if ( type == 2 ) {
				Vector2f v;
				env.addVariable(hash,v);
			}
