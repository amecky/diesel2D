...
void MemoryPool::Destroy(void)
{
    ...
    std::string str;
    if (m_numAllocs != 0)
        str = "***(" + ds::string::to_string(m_numAllocs) + ") ";
    unsigned long totalNumChunks = m_numChunks * m_memArraySize;
    unsigned long wastedMem = (totalNumChunks - m_allocPeak) * m_chunkSize;
    str += "Destroying memory pool: [" + GetDebugName() + ":" + ds::string::to_string((unsigned long)m_chunkSize) + "] = " + ds::string::to_string(m_allocPeak) + "/" + ds::string::to_string((unsigned long)totalNumChunks) + " (" + ds::string::to_string(wastedMem) + " bytes wasted)\n";
    LOG(logINFO) << str; // the logger is not initialized during many of the initial memory pool growths, so let's just use the OS version
#endif
...
{
#ifdef _DEBUG
    std::string str("Growing memory pool: [" + GetDebugName() + ":" + ds::string::to_string((unsigned long)m_chunkSize) + "] = " + ds::string::to_string((unsigned long)m_memArraySize + 1) + "\n");
    LOG(logINFO) << str;  // the logger is not initialized during many of the initial memory pool growths, so let's just use the OS version
#endif
