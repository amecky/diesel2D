...
namespace ds {
	...
	void ParticleSystemEditor::renderGeneratorSettings() {
		if (_generator_selection != -1 && _system != 0) {
			...
			else if (instance.generator->getType() == PGT_ROTATION_VELOCITY) {
				RotationVelocityGeneratorData* data = static_cast<RotationVelocityGeneratorData*>(instance.data);
				gui::Header("Rotation velocity");
				gui::InputFloat("Velocity", &data->velocity);
				gui::InputFloat("Variance", &data->variance);
			}
		}
