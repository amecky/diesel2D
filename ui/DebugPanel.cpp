...
namespace ds {
	...
	DebugPanel::DebugPanel() : _count(0), _state(1) {
	}
	...
	void DebugPanel::showDialog(v2* position) {
		gui::start(1, position);
		char buffer[128];
		if (gui::begin("Debug values", &_state)) {
			for (int i = 0; i < _count; ++i) {
				...
				switch (entry.type) {
					...
					case DebugEntry::DT_VEC3: sprintf_s(buffer, 128, "%s : %3.2f , %3.2f %3.2f", entry.name, entry.values[0], entry.values[1], entry.values[2]); break;
