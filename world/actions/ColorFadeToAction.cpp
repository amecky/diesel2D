	...
	void ColorFadeToAction::update(SpriteArray& array, float dt, ActionEventBuffer& buffer) {
		if ( _buffer.size > 0 ) {				
			...
			for ( int i = 0; i < _buffer.size; ++i ) {
				...
				Color clr;
				clr.r = tweening::interpolate(_tweeningTypes[i], _startColors[i].r, _endColors[i].r, norm);
				clr.g = tweening::interpolate(_tweeningTypes[i], _startColors[i].g, _endColors[i].g, norm);
				clr.b = tweening::interpolate(_tweeningTypes[i], _startColors[i].b, _endColors[i].b, norm);
				clr.a = tweening::interpolate(_tweeningTypes[i], _startColors[i].a, _endColors[i].a, norm);
				sar::setColor(array,_ids[i],clr);
				_timers[i] += dt;
				if ( _timers[i] >= _ttl[i] ) {
					if ( _modes[i] < 0 ) {
						_timers[i] = 0.0f;
